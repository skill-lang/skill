...
parseTypeDescription s = readV64 >>= go s
   where go _ 0  = (return . CInt8)  `fmap` getInt8
         ...
         go _ 5  = return (GRef      `fmap` readV64Pair)  -- Note: the syntax of 0-4 and 5-14 are \not\ interchangeable
         ...
         go _ n  = return ((\index -> GRef ((n - 32), index)) `fmap` readIndex)


readFloat :: Get Float
readFloat = coerce `fmap` getWord32be

readDouble :: Get Double
readDouble = coerce `fmap` getWord64be
