...
{-# NOINLINE states #-}
states :: IORef [State]
states = unsafePerformIO $ newIORef [] -- start with an empty list

initiate = initialize "C:/input/Sen.sf"

createState :: FilePath -> IO State
createState filePath = initialize filePath >> L.last `fmap` readIORef states

readState :: Int -> State
readState index = ((\list -> list !! index) . unsafePerformIO) $ readIORef states

readLatestState :: State
readLatestState = (L.last . unsafePerformIO) $ readIORef states

readTDs :: State -> [TypeDesc]
readTDs (_,_,tDs) = tDs

initialize :: FilePath -> IO ()
initialize filePath = do state <- runGet process `fmap` C.readFile filePath
                         modifyIORef states ((:) state)

initializeTest :: FilePath -> IO ()
initializeTest filePath = initialize filePath >> L.last `fmap` readIORef states >>= printState
...
r'file (f_strings, f_uTs, f_tDs_v2) = isEmpty >>= \e -> if e then return (f_strings, f_uTs, f_tDs_v2)
         ...
         typeData        <- r'data $ (L.map . L.map) (\(_,_,_,offset) -> offset) fDs_v1
...
r'tDs_v1 uTs strings f_tDs_v2 i
   = do name              <- readRefString strings
        let q1             = not $ L.any (\(_,name',_,_,_,_) -> name' == name) f_tDs_v2
        let n_uTs          = select q1 (uTs ++ [name])     uTs
        let id             = select q1 (Just (L.length uTs)) Nothing
        count             <- readV64
...
r'fDs_v1 = go 0
  where go _ _ _ [] = return []
        go latestOffset strings allDescs ((_,name,_,_,_,fieldCount) : r_tDs)
            = do let f_instancesOfCurrentType = L.filter (\(_,name',_,_,_,_) -> name' == name) allDescs
                 let fDsOfCurrentType         = L.concatMap (\(_,_,_,_,_,fDs) -> fDs) f_instancesOfCurrentType
                 let fieldIDsOfCurrentType    = L.map (\(fieldID,_,_,_) -> fieldID) fDsOfCurrentType
...
data TD_v5    = TD5 (Int, String, [FieldDescTest], [TD_v5], [(Int, Int)], Int, Int) deriving (Show) -- id, name, fieldDescs, subtypes, snips, firstIndex, baseFirstIndex
...
compress = L.map (\(Just a1,a2,a3,Just a4,a5,a6) -> (a1, a2, a3, a4, a5, a6))--go []
...
triggerGetters = L.map (\(e1, e2, count, e4, e5, fDs) -> (e1, e2, count, e4, e5, L.map (go count) fDs))
...
createHierarchy typeDescs = L.map convertTD_v5 $ go ([], typeDescs)
   where go :: ([TD_v5], [TD_v4]) -> [TD_v5]
         go (f_tDs, [])                                       = f_tDs
         go (f_tDs, tD : r_tDs)                               = go (f_tDs `integrate` tD, r_tDs)
         convertTD_v5 (TD5 (id, name, e3, children, _, _, _)) = TD (id, name, e3, L.map convertTD_v5 children)
...
integrate f_tDs (id, n, c, sd, lb, fd) = assure (f_tDs `go` (id, n, c, sd, lb, fd)) -- superID > -1 => integrate it, no-failure mode
...
----error $ (show f_tDs) ++ "  " ++ n ++ " " ++ (show id) ++ (show sd)--
 -- \(Just a) -> a)
...
enslave (TD5 (id, name, fDs, tDs, snips, firstIndex, firstBaseIndex)) (id', name', count', _, Just lbpo', fDs')
      ...
      where enslavedTD      = TD5 (id', name', fd2 ++ fDs', [], [], lbpo' + firstBaseIndex, firstBaseIndex)
            (fd1, fd2)      = remodel'' $ L.map (splitFieldDescs adjustedOffsets) fDs
            adjustedOffsets = adjustOffsets (fixedLbpo, count') snips
            fixedLbpo       = lbpo' - firstIndex + 1

adjustOffsets :: (Int, Int) -> [(Int, Int)] -> (Int, Int)
adjustOffsets (offset, count) snips = go (offset, count) $ L.filter (\(offset',_) -> offset' < offset) snips
  where go (offset, count) snips = (offset - sum (L.map snd snips), count)
...
splitFieldDescs (a,b) (name, data', rawData) = ((name, inner, rawData), (name, outer, rawData))
  ...
  where (outer, inner) = (subList a b data', L.take a data' ++ L.drop (a+b) data')
