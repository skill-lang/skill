...
reach :: Int -> [TypeDesc] -> [FieldDescTest]
reach a b = getFDs $ reach'tDs a b
 where reach'tDs index tD = assure $ go index tD
        where go :: Int -> [TypeDesc] -> (Maybe TypeDesc)
              go _ [] = Nothing
              go index (TD (id, name, fDs, s_tDs) : r_f_tDs)
               | id == index = Just (TD (id, name, fDs, s_tDs))
               | otherwise   = case index `go` s_tDs  of Just tD -> Just tD
                                                         Nothing -> Just (index `reach'tDs` r_f_tDs)
fst' (a,b,c) = a
snd' (a,b,c) = b
trd' (a,b,c) = c

getFDs :: TypeDesc -> [FieldDescTest]
getFDs (TD (_, _, fDs, _)) = fDs
