...
\begin{itemize}
 ...
 %restrictions
 \item $\den{r}_{restriction} = \left\{ 
   \begin{array}{l l}
     \emptyset, & r.id = \bot\\
     \den{r.id}_{v64}\den{r.arg_1}_{string}\circ \cdots \circ \den{r.arg_n}_{string} & else
   \end{array} \right.$
 ...
 \item $\den{t}_{type} = \left\{ 
   ...
   \den{21+storagePoolIndex(t)}_{v64} & t \in \mathcal{U}\\
   \end{array} \right.$
\end{itemize}
\textit{id}s of restrictions and types a listed in appendix \ref{app:constants}.

Note that the function $storagePoolIndex$ is implicitly given by the order of storage pools appearing in a file and has therefore to be computed both upon serialization and deserialization.
