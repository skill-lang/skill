    ...
    defs.foreach(d =>
      d.body.foreach(f => {
        ...
        f.t match {
          ...
          case t: SetType => assert(knownTypes.contains(t.baseType), "in " + d.name + "::" + f.name + ":> unknown base type " + t.baseType)
          case t: ListType => assert(knownTypes.contains(t.baseType), "in " + d.name + "::" + f.name + ":> unknown base type " + t.baseType)
          case t: ArrayType => assert(knownTypes.contains(t.baseType), "in " + d.name + "::" + f.name + ":> unknown base type " + t.baseType)
          case t: GroundType => assert(knownTypes.contains(t.name), "in " + d.name + "::" + f.name + ":> unknown base type " + t.name)
        }
