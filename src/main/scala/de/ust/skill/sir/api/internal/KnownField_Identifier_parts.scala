 ...
 */
final class KnownField_Identifier_parts(
  _index : Int,
  _owner : IdentifierPool,
  _type : FieldType[scala.collection.mutable.ArrayBuffer[java.lang.String]])
    extends FieldDeclaration[scala.collection.mutable.ArrayBuffer[java.lang.String],_root_.de.ust.skill.sir.Identifier](_type,
      "parts",
      _index,
      _owner)
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.Identifier].Internal_parts = t.read(in).asInstanceOf[scala.collection.mutable.ArrayBuffer[java.lang.String]]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.Identifier].Internal_parts
