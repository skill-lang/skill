 ...
 */
final class KnownField_Tool_description(
  _index : Int,
  _owner : ToolPool,
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.Tool].Internal_description = t.read(in).asInstanceOf[java.lang.String]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.Tool].Internal_description
          result += t.offset(v)
          i += 1
        }
      case bci : BulkChunk ⇒
