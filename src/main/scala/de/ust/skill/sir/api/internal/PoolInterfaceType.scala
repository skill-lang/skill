...
superPool
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "fields" ⇒ new F_InterfaceType_fields(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.FieldLike]]])
      case "interfaces" ⇒ new F_InterfaceType_interfaces(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.HashSet[_root_.de.ust.skill.sir.InterfaceType]]])
      case "super" ⇒ new F_InterfaceType_super(ID, this, t.asInstanceOf[FieldType[_root_.de.ust.skill.sir.ClassType]])
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val Clsfields = classOf[F_InterfaceType_fields]
    val Clsinterfaces = classOf[F_InterfaceType_interfaces]
    val Clssuper = classOf[F_InterfaceType_super]
    ...
    if(fields.contains(Clsfields))
        dataFields += new F_InterfaceType_fields(dataFields.size + 1, this, VariableLengthArray(state.FieldLike))
    if(fields.contains(Clsinterfaces))
        dataFields += new F_InterfaceType_interfaces(dataFields.size + 1, this, SetType(state.InterfaceType))
    if(fields.contains(Clssuper))
        dataFields += new F_InterfaceType_super(dataFields.size + 1, this, state.ClassType)
    // no auto fields
