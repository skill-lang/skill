 ...
 * bool Field.isAuto
 */
final class KnownField_Field_isAuto(
  _index : Int,
  _owner : FieldPool,
  _type : FieldType[scala.Boolean] = BoolType)
    extends FieldDeclaration[scala.Boolean,_root_.de.ust.skill.sir.Field](_type,
      "isauto",
      _index,
      _owner)
    with KnownField[scala.Boolean,_root_.de.ust.skill.sir.Field] {
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.Field].Internal_isAuto = t.read(in).asInstanceOf[scala.Boolean]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.Field].Internal_isAuto
  ...
  //override def get(i : _root_.de.ust.skill.sir.Field) = i.isAuto
  //override def set(i : _root_.de.ust.skill.sir.Field, v : scala.Boolean) = i.isAuto = v.asInstanceOf[scala.Boolean]

  // note: reflective field access will raise exception for ignored fields
  override def getR(i : SkillObject) : scala.Boolean = i.asInstanceOf[_root_.de.ust.skill.sir.Field].isAuto
  override def setR(i : SkillObject, v : scala.Boolean) : Unit = i.asInstanceOf[_root_.de.ust.skill.sir.Field].isAuto = v.asInstanceOf[scala.Boolean]
}
