 ...
 */
final class KnownField_InterfaceType_super(
  _index : Int,
  _owner : InterfaceTypePool,
  _type : FieldType[_root_.de.ust.skill.sir.ClassType])
    extends FieldDeclaration[_root_.de.ust.skill.sir.ClassType,_root_.de.ust.skill.sir.InterfaceType](_type,
      "super",
      _index,
      _owner)
    with KnownField[_root_.de.ust.skill.sir.ClassType,_root_.de.ust.skill.sir.InterfaceType] {
  ...
  override def read(part : MappedInStream, target : Chunk) {
    ...
    val t = this.t.asInstanceOf[ClassTypePool]
    try {
        target match {
          case c : SimpleChunk ⇒
            var i = c.bpo.toInt
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.InterfaceType].Internal_super = t.read(in).asInstanceOf[_root_.de.ust.skill.sir.ClassType]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.InterfaceType].Internal_super
  ...
  //override def get(i : _root_.de.ust.skill.sir.InterfaceType) = i.`super`
  //override def set(i : _root_.de.ust.skill.sir.InterfaceType, v : _root_.de.ust.skill.sir.ClassType) = i.`super` = v.asInstanceOf[_root_.de.ust.skill.sir.ClassType]

  // note: reflective field access will raise exception for ignored fields
  override def getR(i : SkillObject) : _root_.de.ust.skill.sir.ClassType = i.asInstanceOf[_root_.de.ust.skill.sir.InterfaceType].`super`
  override def setR(i : SkillObject, v : _root_.de.ust.skill.sir.ClassType) : Unit = i.asInstanceOf[_root_.de.ust.skill.sir.InterfaceType].`super` = v.asInstanceOf[_root_.de.ust.skill.sir.ClassType]
}
