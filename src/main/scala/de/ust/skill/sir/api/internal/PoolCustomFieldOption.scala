...
final class CustomFieldOptionPool(poolIndex : Int)
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "arguments" ⇒ new KnownField_CustomFieldOption_arguments(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.ArrayBuffer[java.lang.String]]])
      case "name" ⇒ new KnownField_CustomFieldOption_name(ID, this, t.asInstanceOf[FieldType[java.lang.String]])
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val Clsarguments = classOf[KnownField_CustomFieldOption_arguments]
    val Clsname = classOf[KnownField_CustomFieldOption_name]
    ...
    if(fields.contains(Clsarguments))
        dataFields += new KnownField_CustomFieldOption_arguments(dataFields.size + 1, this, VariableLengthArray(state.String))
    if(fields.contains(Clsname))
        dataFields += new KnownField_CustomFieldOption_name(dataFields.size + 1, this, state.String)
    // no auto fields
