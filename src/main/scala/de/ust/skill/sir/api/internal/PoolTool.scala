...
final class ToolPool(poolIndex : Int)
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "buildtargets" ⇒ new F_Tool_buildTargets(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.BuildInformation]]])
      case "customfieldannotations" ⇒ new F_Tool_customFieldAnnotations(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.HashMap[_root_.de.ust.skill.sir.FieldLike, _root_.de.ust.skill.sir.ToolTypeCustomization]]])
      case "customtypeannotations" ⇒ new F_Tool_customTypeAnnotations(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.HashMap[_root_.de.ust.skill.sir.UserdefinedType, _root_.de.ust.skill.sir.ToolTypeCustomization]]])
      case "description" ⇒ new F_Tool_description(ID, this, t.asInstanceOf[FieldType[java.lang.String]])
      case "name" ⇒ new F_Tool_name(ID, this, t.asInstanceOf[FieldType[java.lang.String]])
      case "selectedfields" ⇒ new F_Tool_selectedFields(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.HashMap[_root_.de.ust.skill.sir.UserdefinedType, scala.collection.mutable.HashMap[java.lang.String, _root_.de.ust.skill.sir.FieldLike]]]])
      case "selectedusertypes" ⇒ new F_Tool_selectedUserTypes(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.HashSet[_root_.de.ust.skill.sir.UserdefinedType]]])
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val ClsbuildTargets = classOf[F_Tool_buildTargets]
    val ClscustomFieldAnnotations = classOf[F_Tool_customFieldAnnotations]
    val ClscustomTypeAnnotations = classOf[F_Tool_customTypeAnnotations]
    val Clsdescription = classOf[F_Tool_description]
    val Clsname = classOf[F_Tool_name]
    val ClsselectedFields = classOf[F_Tool_selectedFields]
    val ClsselectedUserTypes = classOf[F_Tool_selectedUserTypes]
    ...
    if(fields.contains(ClsbuildTargets))
        dataFields += new F_Tool_buildTargets(dataFields.size + 1, this, VariableLengthArray(state.BuildInformation))
    if(fields.contains(ClscustomFieldAnnotations))
        dataFields += new F_Tool_customFieldAnnotations(dataFields.size + 1, this, MapType(state.FieldLike, state.ToolTypeCustomization))
    if(fields.contains(ClscustomTypeAnnotations))
        dataFields += new F_Tool_customTypeAnnotations(dataFields.size + 1, this, MapType(state.UserdefinedType, state.ToolTypeCustomization))
    if(fields.contains(Clsdescription))
        dataFields += new F_Tool_description(dataFields.size + 1, this, state.String)
    if(fields.contains(Clsname))
        dataFields += new F_Tool_name(dataFields.size + 1, this, state.String)
    if(fields.contains(ClsselectedFields))
        dataFields += new F_Tool_selectedFields(dataFields.size + 1, this, MapType(state.UserdefinedType, MapType(state.String, state.FieldLike)))
    if(fields.contains(ClsselectedUserTypes))
        dataFields += new F_Tool_selectedUserTypes(dataFields.size + 1, this, SetType(state.UserdefinedType))
    // no auto fields
