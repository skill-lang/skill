 ...
 */
final class KnownField_FieldView_target(
  _index : Int,
  _owner : FieldViewPool,
  _type : FieldType[_root_.de.ust.skill.sir.FieldLike])
    extends FieldDeclaration[_root_.de.ust.skill.sir.FieldLike,_root_.de.ust.skill.sir.FieldView](_type,
      "target",
      _index,
      _owner)
    with KnownField[_root_.de.ust.skill.sir.FieldLike,_root_.de.ust.skill.sir.FieldView] {
  ...
  override def read(part : MappedInStream, target : Chunk) {
    ...
    val t = this.t.asInstanceOf[FieldLikePool]
    try {
        target match {
          case c : SimpleChunk ⇒
            var i = c.bpo.toInt
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.FieldView].Internal_target = t.read(in).asInstanceOf[_root_.de.ust.skill.sir.FieldLike]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.FieldView].Internal_target
  ...
  //override def get(i : _root_.de.ust.skill.sir.FieldView) = i.target
  //override def set(i : _root_.de.ust.skill.sir.FieldView, v : _root_.de.ust.skill.sir.FieldLike) = i.target = v.asInstanceOf[_root_.de.ust.skill.sir.FieldLike]

  // note: reflective field access will raise exception for ignored fields
  override def getR(i : SkillObject) : _root_.de.ust.skill.sir.FieldLike = i.asInstanceOf[_root_.de.ust.skill.sir.FieldView].target
  override def setR(i : SkillObject, v : _root_.de.ust.skill.sir.FieldLike) : Unit = i.asInstanceOf[_root_.de.ust.skill.sir.FieldView].target = v.asInstanceOf[_root_.de.ust.skill.sir.FieldLike]
}
