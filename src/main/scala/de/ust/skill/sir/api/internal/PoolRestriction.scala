...
final class RestrictionPool(poolIndex : Int)
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "arguments" ⇒ new KnownField_Restriction_arguments(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.ArrayBuffer[java.lang.String]]])
      case "name" ⇒ new KnownField_Restriction_name(ID, this, t.asInstanceOf[FieldType[java.lang.String]])
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val Clsarguments = classOf[KnownField_Restriction_arguments]
    val Clsname = classOf[KnownField_Restriction_name]
    ...
    if(fields.contains(Clsarguments))
        dataFields += new KnownField_Restriction_arguments(dataFields.size + 1, this, VariableLengthArray(state.String))
    if(fields.contains(Clsname))
        dataFields += new KnownField_Restriction_name(dataFields.size + 1, this, state.String)
    // no auto fields
