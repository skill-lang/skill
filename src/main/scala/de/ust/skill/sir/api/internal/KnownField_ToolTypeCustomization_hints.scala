 ...
 */
final class KnownField_ToolTypeCustomization_hints(
  _index : Int,
  _owner : ToolTypeCustomizationPool,
  _type : FieldType[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.Hint]])
    extends FieldDeclaration[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.Hint],_root_.de.ust.skill.sir.ToolTypeCustomization](_type,
      "hints",
      _index,
      _owner)
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.ToolTypeCustomization].Internal_hints = t.read(in).asInstanceOf[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.Hint]]
              i += 1
            }
          case bci : BulkChunk ⇒
