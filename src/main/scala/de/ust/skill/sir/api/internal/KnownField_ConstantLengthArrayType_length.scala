...
/**
 * v64 ConstantLengthArrayType.length
 */
final class KnownField_ConstantLengthArrayType_length(
  _index : Int,
  _owner : ConstantLengthArrayTypePool,
  _type : FieldType[Long] = V64)
    extends FieldDeclaration[Long,_root_.de.ust.skill.sir.ConstantLengthArrayType](_type,
      "length",
      _index,
      _owner)
    with KnownField[Long,_root_.de.ust.skill.sir.ConstantLengthArrayType] {
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.ConstantLengthArrayType].Internal_length = t.read(in).asInstanceOf[Long]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.ConstantLengthArrayType].Internal_length
  ...
  //override def get(i : _root_.de.ust.skill.sir.ConstantLengthArrayType) = i.length
  //override def set(i : _root_.de.ust.skill.sir.ConstantLengthArrayType, v : Long) = i.length = v.asInstanceOf[Long]

  // note: reflective field access will raise exception for ignored fields
  override def getR(i : SkillObject) : Long = i.asInstanceOf[_root_.de.ust.skill.sir.ConstantLengthArrayType].length
  override def setR(i : SkillObject, v : Long) : Unit = i.asInstanceOf[_root_.de.ust.skill.sir.ConstantLengthArrayType].length = v.asInstanceOf[Long]
}
