...
final class FilePathPool(poolIndex : Int)
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "isabsolut" ⇒ new F_FilePath_isAbsolut(ID, this)
      case "parts" ⇒ new F_FilePath_parts(ID, this, t.asInstanceOf[FieldType[scala.collection.mutable.ArrayBuffer[java.lang.String]]])
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val ClsisAbsolut = classOf[F_FilePath_isAbsolut]
    val Clsparts = classOf[F_FilePath_parts]
    ...
    if(fields.contains(ClsisAbsolut))
        dataFields += new F_FilePath_isAbsolut(dataFields.size + 1, this, BoolType)
    if(fields.contains(Clsparts))
        dataFields += new F_FilePath_parts(dataFields.size + 1, this, VariableLengthArray(state.String))
    // no auto fields
