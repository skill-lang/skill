 ...
 */
final class KnownField_CustomField_language(
  _index : Int,
  _owner : CustomFieldPool,
  _type : FieldType[java.lang.String])
    extends KnownField[java.lang.String,_root_.de.ust.skill.sir.CustomField](_type,
      "language",
      _index,
      _owner) {
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.CustomField].Internal_language = t.read(in).asInstanceOf[java.lang.String]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.CustomField].Internal_language
          result += t.offset(v)
          i += 1
        }
      case bci : BulkChunk ⇒
