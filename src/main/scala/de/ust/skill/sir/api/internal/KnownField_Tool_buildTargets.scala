 ...
 */
final class KnownField_Tool_buildTargets(
  _index : Int,
  _owner : ToolPool,
  _type : FieldType[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.BuildInformation]])
    extends FieldDeclaration[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.BuildInformation],_root_.de.ust.skill.sir.Tool](_type,
      "buildtargets",
      _index,
      _owner)
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.Tool].Internal_buildTargets = t.read(in).asInstanceOf[scala.collection.mutable.ArrayBuffer[_root_.de.ust.skill.sir.BuildInformation]]
              i += 1
            }
          case bci : BulkChunk ⇒
