...
superPool
  ...
  override def addField[T : Manifest](ID : Int, t : FieldType[T], name : String,
    ...
    val f = (name match {
      case "length" ⇒ new F_ConstantLengthArrayType_length(ID, this)
  ...
  override def ensureKnownFields(st : SkillState) {
    ...
    // data fields
    val Clslength = classOf[F_ConstantLengthArrayType_length]
    ...
    if(fields.contains(Clslength))
        dataFields += new F_ConstantLengthArrayType_length(dataFields.size + 1, this, V64)
    // no auto fields
