...
/**
 * type TypeDefinition.target
 */
final class KnownField_TypeDefinition_target(
  _index : Int,
  _owner : TypeDefinitionPool,
  _type : FieldType[_root_.de.ust.skill.sir.Type])
    extends FieldDeclaration[_root_.de.ust.skill.sir.Type,_root_.de.ust.skill.sir.TypeDefinition](_type,
      "target",
      _index,
      _owner)
    ...
    try {
        target match {
          case c : SimpleChunk ⇒
            ...
            val high = i + c.count
            while (i != high) {
              d(i).asInstanceOf[_root_.de.ust.skill.sir.TypeDefinition].Internal_target = t.read(in).asInstanceOf[_root_.de.ust.skill.sir.Type]
              i += 1
            }
          case bci : BulkChunk ⇒
  ...
  def offset: Unit = {
    ...
    dataChunks.last match {
      case c : SimpleChunk ⇒
        ...
        val high = i + c.count
        while (i != high) {
          val v = data(i).asInstanceOf[_root_.de.ust.skill.sir.TypeDefinition].Internal_target
