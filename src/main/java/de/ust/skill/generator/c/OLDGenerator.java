...
public class OLDGenerator {
    ...
    private final List<UserType> declarations;
    ...
    public OLDGenerator(String outPath, String prefix, List<UserType> declarations, boolean safe) {
        ...
        try {
            configuration.setDirectoryForTemplateLoading(new File("templates/c/"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public List<UserType> getDeclarations() {
        return declarations;
    }
    ...
    public void make() throws Exception {
        ...
        writeFile(outPathApi + File.separator + prefix + "api.h", "api-h.ftl", prefix);
        writeFile(outPathApi + File.separator + prefix + "api.c", "api-c.ftl", prefix);

        /**
         * io
         */
        String outPathIo = outPath + File.separator + "io";
        writeFile(outPathIo + File.separator + prefix + "reader.h", "reader-h.ftl", prefix);
        writeFile(outPathIo + File.separator + prefix + "reader.c", "reader-c.ftl", prefix);

        writeFile(outPathIo + File.separator + prefix + "writer.h", "writer-h.ftl", prefix);
        writeFile(outPathIo + File.separator + prefix + "writer.c", "writer-c.ftl", prefix);

        writeFile(outPathIo + File.separator + prefix + "binary_reader.h", "binary-reader-h.ftl", prefix);
        writeFile(outPathIo + File.separator + prefix + "binary_reader.c", "binary-reader-c.ftl", prefix);

        writeFile(outPathIo + File.separator + prefix + "binary_writer.h", "binary-writer-h.ftl", prefix);
        writeFile(outPathIo + File.separator + prefix + "binary_writer.c", "binary-writer-c.ftl", prefix);

        /**
         * model
         */
        String outPathModel = outPath + File.separator + "model";
        writeFile(outPathModel + File.separator + prefix + "string_access.h", "string-access-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "string_access.c", "string-access-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "skill_state.h", "skill-state-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "skill_state.c", "skill-state-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "type_information.h", "type-information-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "type_information.c", "type-information-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "type_declaration.h", "type-declaration-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "type_declaration.c", "type-declaration-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "type_enum.h", "type-enum-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "type_enum.c", "type-enum-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "storage_pool.h", "storage-pool-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "storage_pool.c", "storage-pool-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "field_information.h", "field-information-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "field_information.c", "field-information-c.ftl", prefix);

        writeFile(outPathModel + File.separator + prefix + "types.h", "types-h.ftl", prefix);
        writeFile(outPathModel + File.separator + prefix + "types.c", "types-c.ftl", prefix);

        /**
         * makefile
         */
        String outPathMakefile = outPath;
        writeFile(outPathMakefile + File.separator + "makefile", "makefile.ftl", prefix);
    }

    private void setTypeFields(Map<String, Object> map, Type source, boolean isConstant) {
        ...
        if (source instanceof Declaration) {
            map.put("declaration_type_name", ((Declaration) source).getName().cStyle());
        }
        ...
        map.put("is_constant_length_array", source instanceof ConstantLengthArrayType);
        map.put("is_variable_length_array", source instanceof VariableLengthArrayType);
    ...
    private Map<String, Object> createFieldMap(Field source, Declaration declaration) {
        Map<String, Object> result = new HashMap<>();
        result.put("name", source.getName().cStyle());
        ...
        result.put("declaration_name", declaration.getName().cStyle());
    ...
    // fields
    private List<Map<String, Object>> createFieldsList(UserType source) {
    ...
    private List<Map<String, Object>> createConstantFieldsList(UserType source) {
        List<Map<String, Object>> result = new ArrayList<>();
    ...
    private boolean containsStringFields(Field field) {
        ...
        if (type instanceof ContainerType) {
            if (type instanceof ConstantLengthArrayType) {
                if (((ConstantLengthArrayType) type).getBaseType().getName().equals("string")) {
                    return true;
                }
            } else if (type instanceof VariableLengthArrayType) {
                if (((VariableLengthArrayType) type).getBaseType().getName().equals("string")) {
                    return true;
                }
    ...
    private boolean containsStringFields(UserType declaration) {
    ...
    private void createRootMap(List<UserType> declarations, boolean safe) {
        ...
        for (UserType currentDeclaration : declarations) {
        ...
        for (UserType currentDeclaration : declarations) {
            ...
            declarationMap.put("name", currentDeclaration.getName().cStyle());

            Type superType = currentDeclaration.getSuperType();
            if (superType == null) {
                declarationMap.put("super_type", "skill_type");
            } else {
                declarationMap.put("super_type", superType.getName().cStyle());
            }
            ...
            declarationMap.put("contains_strings", containsStringFields(currentDeclaration));
            ...
            declarationMap.put("fields_including_constants", constantFieldsList);

            // contains all fields inherited from super types, but no constant
            // fields
            List<Map<String, Object>> superFieldsList = new ArrayList<>();
            UserType superDeclaration = currentDeclaration.getSuperType();
            ...
            UserType superConstantDeclaration = currentDeclaration.getSuperType();
            while (superConstantDeclaration != null) {
                constantSuperFieldsList.addAll(createConstantFieldsList(superConstantDeclaration));
                superConstantDeclaration = superConstantDeclaration.getSuperType();
            }
            declarationMap.put("super_fields_including_constants", constantSuperFieldsList);
            ...
            declarationMap.put("all_fields_including_constants", allConstantFieldsList);
    ...
    private void writeFile(String outFile, String templateName, String packagePrefix) throws IOException,
            TemplateException {
    ...
    private String typeToCType(Type type) {
        if (type instanceof GroundType) {
            switch (type.getName().getSkillName()) {
            case "annotation":
            ...
            return this.prefix + ((Declaration) type).getName().cStyle();
        } else {
            return "unknown type";
        }
        return null;
    ...
    private String typeToEnumType(Type type) {
        if (type instanceof ConstantLengthArrayType) {
            ...
            case "bool":
                return "BOOLEAN";
            default:
                return type.getName().cStyle().toUpperCase();
            }
        }
        return null;
    }
