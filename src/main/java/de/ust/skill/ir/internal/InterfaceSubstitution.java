...
public class InterfaceSubstitution extends Substitution {
    ...
    public void initialize(TypeContext fromTC, TypeContext tc, UserType d) throws ParseException {
        ...
        // collect customizations from super interfaces
        List<LanguageCustomization> customs = new ArrayList<>(t.getCustomizations());
        for (InterfaceType i : t.getSuperInterfaces())
            addCustomsRecursive(tc, customs, i);

        UserType superType = findSuperType(t);

        d.initialize((UserType) substitute(tc, superType), Collections.emptyList(), fields, t.getViews(), customs);
    }
    ...
    private void addCustomsRecursive(TypeContext tc, List<LanguageCustomization> fields, InterfaceType i)
            throws ParseException {
        fields.addAll(i.getCustomizations());
        for (InterfaceType sub : i.getSuperInterfaces())
            addCustomsRecursive(tc, fields, sub);
    }
