...
/**
 * Type substitution that can be used to modify a type context. The substitution provides a predicate drop, that decides
 * whether or not to drop a type in the substitution and a type replacement predicate that will be used in the process
 * of cloning a state to replace types by other types.
 * 
 * @author Timm Felden
 */
abstract public class Substitution {
    ...
    /**
     * Substitution is done on fields, because some type information(restrictions/hints) is part of the field and not of
     * the type itself. Furthermore, fields with a dropped type will not be dropped!
     * 
     * @note the substitution will copy the type of the argument field by using types available in the argument type
     *       context
     */
