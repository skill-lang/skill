#ifndef ${prefix_capital}STORAGE_POOL_H_
#define ${prefix_capital}STORAGE_POOL_H_
...
// Instances which are created by the user, but are not yet serialized, do not belong to a type block.
typedef struct ${prefix}storage_pool_struct {
    int64_t id; // this is the id, this pool has in the binary file (defined by the order, in which the types appear in the file)
                // if this pool has not yet been written to file, this is set to -1.
    bool declared_in_file; // This is set to true, if this pool has already been written to a binary file.
    ${prefix}type_declaration declaration;
    GList *fields; // list of field_information. This only contains fields from this pool's declaration
                   // This is empty, unless this pool has already been written to file.
                   // Unknown fields are not stored at the moment and they will not be written when appending.
    
    ${prefix}storage_pool base_pool;  // This is the root of the inheritance tree. May point to itself.
    ${prefix}storage_pool super_pool; // This is the direct super-type
    GList *sub_pools;                  // GList of ${prefix}storage_pool. These are only the direct sub-types.
    int64_t lbpsi;                     // If this is a subtype, this field will be set before writing/appending.
                                       // This is the index of the first instance of this type in the instance array
                                       // of the base type.

    GArray *instances;     // Array of skill_type*. This only includes instances of this exact type. Instances of Sub-Types will be 
                           // hold by the sub-pools. This contains instances, that are already contained in a binary file.
    GArray *new_instances; // Array of skill_type*. This only includes instances of this exact type. Instances of Sub-Types will be 
                           // hold by the sub-pools. This contains only instances, that are user-created and not contained in a binary file.
...
// Returns all instances of this type (including sub-types). The list must be deallocated manually.
GList *${prefix}storage_pool_get_instances ( ${prefix}storage_pool this );

// Returns instances of this type (including sub-types), that are created by the user, and not yet appended to the binary file.
